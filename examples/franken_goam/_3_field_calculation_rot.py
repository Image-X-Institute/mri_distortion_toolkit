...
# data_loc = Path(r'/home/brendan/HairyHome/Downloads/FrankenGoam^Mr/20221107 MR Linac^Test')
...
matched_volume = pd.read_csv(data_csv_loc / 'rot_markers.csv', index_col=0).squeeze("columns")
dicom_data = data_loc / scans['17'] / 'Original' / 'dicom_data.json'  # previously saved from a MarkerVolume
Bz_field = ConvertMatchedMarkersToBz(matched_volume, dicom_data)
Bz_field.MagneticFields.to_csv(data_csv_loc / 'rot_Bfields.csv')