...
distorted_data_loc = data_loc / '10 t1_tse_256_sag' / 'Original'
...
"""
- so: Gz harmonics are fine.
- y - makes things a bit worse, still not terrible. y is frequency encode.
- z - not terrible.
- so it seems the major difference in y...
"""

GDC.correct_all_images()
# GDC.save_all_images(DSV_radius=None)  # saves as png so you can quickly inspect results
GDC.save_all_images_as_dicom()  # saves as dicom which can be read into analysis packages.

# Assess correction:
corrected_volume = MarkerVolume(distorted_data_loc / 'corrected_dcm', n_markers_expected=618,
...
matched_volume_corrected = MatchedMarkerVolumes(gt_volume,
                                                corrected_volume,
                                                n_refernce_markers=9)
